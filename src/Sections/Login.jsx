            ...
            <section>   <main>
                <div className="w-full min-h-screen overflow-y-auto bgblur bg-cover flex items-center justify-center p-4 sm:p-6 fixed inset-0 z-50 bg-[#0c0c13ad]">
                    <div className='relative w-full max-w-md max-h-[calc(100vh-2rem)] overflow-y-auto flex flex-col bg-gray-950 shadow-gray-600 shadow-2xl'>
                        <div className=' text-[17px] relative w-full text-end top-0 right-0'>
                            <button type='button' aria-label='Close registration' className='bg-[#4ddb9e] pl-4 pr-1 pb-2 cursor-pointer rounded-bl-[100%] text-white' onClick={onclick} >
                        ...
                        <div className='flex flex-col gap-4 px-5 py-8 sm:px-10 sm:py-10'>
                            ...
                            <div className='w-full py-3'>
                                <form className='flex w-full flex-col gap-5' onSubmit={handle_login_data}>
                                    <label className='flex flex-col gap-1 text-xs  uppercase tracking-wider text-gray-400' htmlFor='user_name'>
                                        Name 
                                        <Input type="text" name="user_name" placeholder={'Your name'} className={`${input_style}`} value={user_name} onChange={(e) => setuser_name(e.target.value)} />
                                    </label>

                                    <label className='flex flex-col gap-1 text-xs  uppercase tracking-wider text-gray-400' htmlFor='user_email'>Email
                                        <Input type="email" name="email" placeholder={'you@example.com'} className={`${input_style}`}
                                            value={user_email} onChange={(e) => setuser_email(e.target.value)} />
                                    </label>

                                    <label className='flex flex-col gap-1 text-xs uppercase tracking-wider text-gray-400' htmlFor='user_password'>Password
                                        <Input type="password" name="password" placeholder='8+ characters' className={`${input_style}`} value={user_password} onChange={(e) => setuser_password(e.target.value)} />
                                    </label>
